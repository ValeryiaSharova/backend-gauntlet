...
const parseConnectionString = (connection: string) => {
...
export const dump = (url: string, withoutData?: boolean): string => {
  const { user, password, host, port, database } = parseConnectionString(url);

  const args =
    '--rows-per-insert=100 --no-owner --no-acl --disable-triggers' +
    ' --no-comments --no-publications --no-security-labels --no-subscriptions' +
    " --no-tablespaces --exclude-table='pgmigration*' --exclude-schema=utils" +
    (withoutData ? ' --schema-only' : '');
...
export const testServer = async ({
  ...
  const DB_PORT = parseConnectionString(
